...
const TARGET = apiUrl("items/user");
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  const url = new URL(req.url);
  // Support /api/user?id=123 or /api/user&user_id=123 style by mapping to /items/user/<id>
  const id = url.searchParams.get("id") || url.searchParams.get("user_id");
  if (id) {
    return forward("DELETE", req, idPathUrl(id));
  }
  // If no id, forward as-is to backend (if it supports bulk delete)
