...
export function CustomerDiscountCategoryFormDialog({
  open,
  onClose,
  onSubmit,
  customer,
  ...
  useEffect(() => {
    if (open) {
      // @ts-ignore
        provider.listCategories([]).then(setCategories);
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Add Category Discount for ${customer.customer_name}`}>
      <div className="space-y-4 p-4">
        <div>
          ...
          >
            ...
            {categories.map((c) => (
              <option key={`category-${c.category_id}`} value={c.category_id}>
                {c.category_name}
              </option>
            ))}
          </select>
